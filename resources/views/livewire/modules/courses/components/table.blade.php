<div>
    <div class="bg-white border text-gray-500 shadow-lg  rounded-xl border-gray-300">
        ...
        <div class="flex items-center justify-between space-x-2 p-3">
            <div>
                <p class="text-sm text-gray-500"> Menampilkan {{ $courses->currentPage() }} to {{ $courses->lastPage() }} dari
                    {{ $courses_count }} hasil
                </p>
            ...
            <div>
                ...
                {{-- Previous --}}
                <button wire:click="previousPage" @disabled($courses->onFirstPage()) class="px-3 py-1 border border-gray-300 rounded-md text-sm hover:bg-gray-100 transition disabled:opacity-50">
                    Previous
                </button>
            ...
            {{ $i == $current ? 'bg-blue-500 text-white' : 'border border-gray-300 hover:bg-gray-100' }}">
                ...
                {{-- Next --}}
                <button wire:click="nextPage" @disabled(!$courses->hasMorePages()) class="px-3 py-1 border border-gray-300 rounded-md text-sm hover:bg-gray-100 transition disabled:opacity-50">
                    Next
                </button>
